...
| **Claude Platform on AWS** | One provider and two models rather than a routing layer; native source citations and prompt caching |
...
- **Server API key** (`X-API-Key` header): uses the server's own Claude Platform key with the community's default model
- **BYOK (Bring Your Own Key)**: the caller supplies a key on `X-Anthropic-API-Key` or `X-OpenRouter-Key`,
